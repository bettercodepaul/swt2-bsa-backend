 ...
 **/
CREATE TABLE region (
  regionId             DECIMAL(19,0) NOT NULL    DEFAULT nextval('sq_region_id'), -- DECIMAL(19,0) = unsigned long
  ...
  CONSTRAINT uc_region_name UNIQUE (regionName),
  CONSTRAINT uc_region_kuerzel UNIQUE (region_kuerzel),

  -- foreign key (fk)
  -- schema: fk_{current table name}_{foreign key origin table name}
  CONSTRAINT fk_region_region FOREIGN KEY (region_uebergeordnet) REFERENCES region (regionId)
