package de.bogenliga.application.business.wettkampftyp.impl.mapper;


import java.sql.Timestamp;
import java.time.OffsetDateTime;
import java.util.function.Function;
import de.bogenliga.application.business.wettkampftyp.api.types.WettkampftypDO;
import de.bogenliga.application.business.wettkampftyp.impl.entity.WettkampftypBE;
import de.bogenliga.application.common.component.mapping.ValueObjectMapper;
import de.bogenliga.application.common.time.DateProvider;

/**
 * I convert the wettkampf DataObjects and BusinessEntities.
 *
 */
public class WettkampftypMapper implements ValueObjectMapper {

    /**
     * Converts a {@link WettkampftypBE} to a {@link WettkampftypDO}
     *
     */
    public static final Function<WettkampftypBE, WettkampftypDO> toWettkampftypDO = be -> {


        final Long wettkampftypId = be.getWettkampftypID();
        final String wettkampftypName = be.getWettkampftypName();

        // technical parameter
        Long createdByUserId = be.getCreatedByUserId();
        Long lastModifiedByUserId = be.getLastModifiedByUserId();
        Long version = be.getVersion();

        OffsetDateTime createdAtUtc = DateProvider.convertTimestamp(be.getCreatedAtUtc());
        OffsetDateTime lastModifiedAtUtc = DateProvider.convertTimestamp(be.getLastModifiedAtUtc());

        return new WettkampftypDO(wettkampftypId, wettkampftypName,
                createdAtUtc, createdByUserId, lastModifiedAtUtc, lastModifiedByUserId, version);
    };

    /**
     * Converts a {@link WettkampftypDO} to a {@link WettkampftypBE}
     */
    public static final Function<WettkampftypDO, WettkampftypBE> toWettkampftypBE = wettkampfDO -> {

        Timestamp createdAtUtcTimestamp = DateProvider.convertOffsetDateTime(wettkampfDO.getCreatedAtUtc());
        Timestamp lastModifiedAtUtcTimestamp = DateProvider.convertOffsetDateTime(wettkampfDO.getLastModifiedAtUtc());

        WettkampftypBE wettkampftypBe = new WettkampftypBE();
        wettkampftypBe.setWettkampftypID(wettkampfDO.getWettkampftypId());
        wettkampftypBe.setWettkampftypName(wettkampfDO.getWettkampftypName());


        wettkampftypBe.setCreatedAtUtc(createdAtUtcTimestamp);
        wettkampftypBe.setCreatedByUserId(wettkampfDO.getCreatedByUserId());
        wettkampftypBe.setLastModifiedAtUtc(lastModifiedAtUtcTimestamp);
        wettkampftypBe.setLastModifiedByUserId(wettkampfDO.getLastModifiedByUserId());
        wettkampftypBe.setVersion(wettkampfDO.getVersion());

        return wettkampftypBe;
    };


    /**
     * Private constructor
     */
    private WettkampftypMapper() {
        // empty private constructor
    }
}
