...
public class UserService implements ServiceFacade {
    ...
    private static final String PRECONDITION_MSG_USER_PASSWORD = "This is not a valid Password";

    private static final String PASSWORD_VALIDATION_REGEX = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)[A-Za-z\\d#$^+=!*()@%&?]{8,}$";
    ...
    public UserDTO create(final HttpServletRequest requestWithHeader,
        ...
        Preconditions.checkArgument(userCredentialsDTO.getPassword().matches(PASSWORD_VALIDATION_REGEX), PRECONDITION_MSG_USER_PASSWORD);
